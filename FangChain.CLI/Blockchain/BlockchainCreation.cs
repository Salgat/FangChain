...
namespace FangChain.CLI
{
    ...
    {
        public async Task CreateBlockChainAsync(DirectoryInfo blockchainDirectory, string credentialsPath)
        {
            var keysJson = await File.ReadAllTextAsync(credentialsPath);
        ...
        public async Task PersistBlockAsync(DirectoryInfo blockchainDirectory, BlockModel block)
        {
            ...
            await File.WriteAllTextAsync(destination, blockJson);
        }

        public async Task<BlockModel> ReadBlockAsync(string blockPath)
        {
            var blockJson = await File.ReadAllTextAsync(blockPath);
